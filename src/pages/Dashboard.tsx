    ...
    <PageTransition className="h-screen">
      <div className="flex h-full flex-col bg-background">
        ...
        <motion.header
          ...
          className="flex items-center justify-between border-b border-border/50 glass-strong px-3 md:px-4 py-2.5 shrink-0 relative z-20"
        >
          <div className="flex items-center gap-2 md:gap-3">
            <Link to="/dashboard" className="flex items-center gap-2 hover:opacity-90 transition-opacity group">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 border border-primary/20 group-hover:glow-subtle transition-all duration-300">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary transition-colors" onClick={() => setShowHelp(true)} title="Help">
              <HelpCircle className="h-4 w-4" />
            </Button>
            {!isMobile && (
              <>
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary transition-colors" onClick={() => setShowLeft(!showLeft)}>
                  {showLeft ? <PanelLeftClose className="h-4 w-4" /> : <PanelLeft className="h-4 w-4" />}
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary transition-colors" onClick={() => setShowRight(!showRight)}>
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-destructive/10 hover:text-destructive transition-colors" onClick={signOut}>
        ...
        {/* Mobile bottom nav */}
        {isMobile && (
          <div className="order-last border-t border-border/50 glass-strong flex shrink-0 relative z-20">
            {[
              ...
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-1 py-2.5 text-xs transition-all duration-200 ${
                  mobileTab === tab.id ? "text-primary" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                <tab.icon className={`h-4 w-4 transition-transform duration-200 ${mobileTab === tab.id ? "scale-110" : ""}`} />
          ...
          <AnimatePresence>
            {(isMobile ? mobileTab === "timeline" : showLeft) && (
              <motion.aside
                ...
                className={`${isMobile ? "w-full" : "w-72"} shrink-0 border-r border-border/30 bg-card/20`}
              >
                <MemoryTimeline
                  memories={memories}
          ...
          {(isMobile ? mobileTab === "main" : true) && (
            <main className="flex-1 flex flex-col min-w-0">
              <Tabs defaultValue="chat" className="flex flex-1 flex-col">
                <div className="border-b border-border/30 px-3 md:px-4 glass-strong">
                  <TabsList className="bg-transparent h-10">
                    <TabsTrigger value="chat" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary text-xs md:text-sm transition-all duration-200">
                      <MessageSquare className="h-3.5 w-3.5" /> Chat
                    </TabsTrigger>
                    <TabsTrigger value="add" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary text-xs md:text-sm transition-all duration-200">
          ...
          <AnimatePresence>
            {(isMobile ? mobileTab === "insights" : showRight) && (
              <motion.aside
                ...
                exit={{ opacity: 0, x: 20 }}
                transition={{ duration: 0.25, ease: "easeOut" }}
                className={`${isMobile ? "w-full" : "w-72"} shrink-0 border-l border-border/30 bg-card/20`}
              >
                <div className="border-b border-border/30 p-4 glass-strong">
        ...
        <Sheet open={!!selectedMemory && !isEditing} onOpenChange={(open) => { if (!open) { setSelectedMemory(null); setIsEditing(false); } }}>
          <SheetContent className="sm:max-w-lg w-[90vw] glass-strong">
            <SheetHeader>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <SheetDescription>
                    <div className="flex gap-2 mt-1 flex-wrap">
                      <Badge variant="outline" className="border-primary/30 text-primary">{selectedMemory?.type}</Badge>
                      <Badge variant="outline" className="border-accent/30 text-accent">{selectedMemory?.memory_layer}</Badge>
                      {selectedMemory && (
                ...
                <Button variant="outline" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary hover:border-primary/30 transition-all" onClick={() => setIsEditing(true)}>
          ...
          <DialogContent className="sm:max-w-lg w-[95vw]">
            ...
            <ScrollArea className="max-h-[60vh] pr-4">
              <div className="space-y-4 text-sm">
                <div>
                  <h3 className="font-semibold mb-2 text-primary">Adding Memories</h3>
                ...
                <div>
                  <h3 className="font-semibold mb-2 text-primary">Chat with Your Memories</h3>
                  <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                ...
                <div>
                  <h3 className="font-semibold mb-2 text-primary">Search & Filter</h3>
                  <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                ...
                <div className="pt-2 border-t border-border/30">
                  <p className="text-xs text-muted-foreground">
                    <strong className="text-primary">Tip:</strong> Use "Load Demo Data" to try the app with example memories!
                  </p>
                </div>
              </div>
            </ScrollArea>
